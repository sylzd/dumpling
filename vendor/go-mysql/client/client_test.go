...
func TestConn(t *testing.T) {
	conn, _ := Connect("tj1-test-ygc.kscn:3306", "midts_test1_wr", "Di8Epi0dKxCbu1n4", "ygc_test")

	conn.Ping()
	//priKeyQuery := "SELECT column_name FROM information_schema.columns  WHERE table_schema = \"ygc_test\" AND table_name = \"test02\" AND column_key = 'PRI';"
	priKeyQuery := "SELECT * FROM ygc_test.test01   limit 1;"
	// Select
	r, err := conn.Execute(priKeyQuery)
	if err!=nil{
		fmt.Println("show create database err.")
	}
	...
	for _, row := range r.Values {
		for _, val := range row {
			//res = val.Value() // interface{}
			//// or

			if val.Type == mysql.FieldValueTypeFloat {
				res := val.AsFloat64() // float64
				fmt.Println("res 0 ", res)

			}
			if val.Type == mysql.FieldValueTypeString {
				res := val.AsString() // float64
				fmt.Println("res 1 ", string(res))
			}
		}
	}
}
...
func TestConn2(t *testing.T) {
	...
	rows, err:=conn.Query(priKeyQuery)
	if err!=nil{
		fmt.Println(err)
	}
	rows.Start()
	fmt.Println(rows.Status)
